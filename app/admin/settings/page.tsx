...
import { Save, Bell, CreditCard, Mail, Truck, Shield, Globe, Eye, EyeOff, CheckCircle, Building2, Loader } from "lucide-react";
...
export default function SettingsPage() {
  ...
  const [settings, setSettings] = useState({
    ...
    businessPhone:         "+234 703 100 2094",
    businessAddress:       "Lagos, Nigeria",
    businessWebsite:       "https://luluartistry.store",
    bankName:              "GTBank",
    ...
    standardShippingCost:  "1200",
    expressShippingCost:   "2500",
      ...
      try {
        ...
        if (json.success && json.data) {
          ...
          setSettings(prev => ({
            ...
            ...(s.bank || {}),
            ...(s.shipping ? {
              standardShippingCost:  String(s.shipping.standardShippingCost || prev.standardShippingCost),
              expressShippingCost:   String(s.shipping.expressShippingCost || prev.expressShippingCost),
              freeShippingThreshold: String(s.shipping.freeShippingThreshold || prev.freeShippingThreshold),
              pickupAvailable:       s.shipping.pickupAvailable ?? prev.pickupAvailable,
            } : {}),
            ...(s.notifications || {}),
          }));
        }
  ...
  const handleSave = async (section: string) => {
    ...
    try {
      const sectionMap: Record<string, any> = {
        ...
        Shipping: {
          ...
          data: {
            ...
            expressShippingCost:   Number(settings.expressShippingCost),
      ...
      const payload = sectionMap[section];
      if (!payload) {
  ...
  const handleChangePassword = async () => {
    ...
    try {
      const res = await fetch(`${BASE_URL}/auth/update-password`, {
        ...
        body: JSON.stringify({ currentPassword: settings.currentPassword, newPassword: settings.newPassword })
      });
      const json = await res.json();
  ...
  const SecretInput = ({ field, label, placeholder }: { field: string; label: string; placeholder?: string }) => (
    <div>
      ...
      <div className="relative">
        <input type={showSecrets[field] ? "text" : "password"} value={(settings as any)[field]}
  ...
  const Toggle = ({ field, label, description }: { field: string; label: string; description?: string }) => (
    <div className="flex items-center justify-between p-4 bg-gray-50 rounded-lg">
      ...
      <button onClick={() => set(field, !(settings as any)[field])}
        className={`relative w-11 h-6 rounded-full transition-colors ${(settings as any)[field] ? "bg-yellow-500" : "bg-gray-300"}`}>
        <span className={`absolute top-0.5 left-0.5 w-5 h-5 bg-white rounded-full shadow transition-transform ${(settings as any)[field] ? "translate-x-5" : "translate-x-0"}`} />
      </button>
    </div>
  );
      ...
      <div className="flex flex-col lg:flex-row gap-6">
              ...
              return (
                <button key={tab.id} onClick={() => setActiveTab(tab.id)}
                  className={`w-full flex items-center gap-3 px-4 py-3.5 text-sm font-medium transition-colors border-b border-gray-100 last:border-0 ${
                    activeTab === tab.id ? "bg-yellow-50 text-yellow-700 border-l-4 border-l-yellow-500" : "text-gray-600 hover:bg-gray-50"
                  }`}>
        ...
        <div className="flex-1">
          ...
          {activeTab === "general" && (
            <div className="bg-white rounded-xl border border-gray-200 shadow-sm p-6">
              <div className="flex items-center justify-between mb-6">
                <div><h2 className="text-lg font-bold text-gray-900">General Settings</h2><p className="text-sm text-gray-500">Basic business information</p></div>
                <SaveButton section="General" />
              </div>
              <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                <div><label className={labelClass}>Business Name</label><input type="text" value={settings.businessName} onChange={e => set("businessName", e.target.value)} className={inputClass} /></div>
                <div><label className={labelClass}>Business Email</label><input type="email" value={settings.businessEmail} onChange={e => set("businessEmail", e.target.value)} className={inputClass} /></div>
                <div><label className={labelClass}>Phone Number</label><input type="tel" value={settings.businessPhone} onChange={e => set("businessPhone", e.target.value)} className={inputClass} /></div>
                <div><label className={labelClass}>Website</label><input type="url" value={settings.businessWebsite} onChange={e => set("businessWebsite", e.target.value)} className={inputClass} /></div>
                <div className="md:col-span-2"><label className={labelClass}>Business Address</label><input type="text" value={settings.businessAddress} onChange={e => set("businessAddress", e.target.value)} className={inputClass} /></div>
              </div>
            </div>
          )}
          ...
          {activeTab === "bank" && (
            <div className="bg-white rounded-xl border border-gray-200 shadow-sm p-6">
              <div className="flex items-center justify-between mb-6">
                <div><h2 className="text-lg font-bold text-gray-900">Bank Account Details</h2><p className="text-sm text-gray-500">Used for bank transfer payments</p></div>
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                <div><label className={labelClass}>Bank Name</label><input type="text" value={settings.bankName} onChange={e => set("bankName", e.target.value)} className={inputClass} /></div>
                <div><label className={labelClass}>Account Number</label><input type="text" value={settings.accountNumber} onChange={e => set("accountNumber", e.target.value)} className={inputClass} /></div>
                <div><label className={labelClass}>Account Name</label><input type="text" value={settings.accountName} onChange={e => set("accountName", e.target.value)} className={inputClass} /></div>
                <div><label className={labelClass}>Branch (Optional)</label><input type="text" value={settings.bankBranch} onChange={e => set("bankBranch", e.target.value)} placeholder="e.g. Victoria Island" className={inputClass} /></div>
              </div>
          ...
          {activeTab === "payment" && (
            <div className="bg-white rounded-xl border border-gray-200 shadow-sm p-6">
              <div className="flex items-center justify-between mb-6">
                <div><h2 className="text-lg font-bold text-gray-900">Payment Gateway Keys</h2><p className="text-sm text-gray-500">API keys for payment processing</p></div>
              ...
              <div className="space-y-4">
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                  <div><label className={labelClass}>Public Key</label><input type="text" value={settings.paystackPublicKey} onChange={e => set("paystackPublicKey", e.target.value)} placeholder="pk_test_..." className={inputClass} /></div>
                ...
                <div className="border-t border-gray-100 pt-4">
                  ...
                  <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                    <div><label className={labelClass}>Public Key</label><input type="text" value={settings.flutterwavePublicKey} onChange={e => set("flutterwavePublicKey", e.target.value)} placeholder="FLWPUBK_..." className={inputClass} /></div>
          ...
          {activeTab === "shipping" && (
            <div className="bg-white rounded-xl border border-gray-200 shadow-sm p-6">
              <div className="flex items-center justify-between mb-6">
                <div><h2 className="text-lg font-bold text-gray-900">Shipping Settings</h2><p className="text-sm text-gray-500">Configure delivery costs and options</p></div>
          ...
          {activeTab === "notifications" && (
            <div className="bg-white rounded-xl border border-gray-200 shadow-sm p-6">
              <div className="flex items-center justify-between mb-6">
                <div><h2 className="text-lg font-bold text-gray-900">Notification Settings</h2><p className="text-sm text-gray-500">Control when and how you receive alerts</p></div>
          ...
          {activeTab === "security" && (
            <div className="bg-white rounded-xl border border-gray-200 shadow-sm p-6">
              <div className="mb-6"><h2 className="text-lg font-bold text-gray-900">Security Settings</h2><p className="text-sm text-gray-500">Update your admin password</p></div>
              <div className="max-w-md space-y-4">
                <SecretInput field="currentPassword" label="Current Password" placeholder="Enter current password" />
                <SecretInput field="newPassword" label="New Password" placeholder="Enter new password (min 8 characters)" />
                ...
                {settings.newPassword && settings.confirmPassword && (
                  <div className={`flex items-center gap-2 text-sm ${settings.newPassword === settings.confirmPassword ? "text-green-600" : "text-red-500"}`}>
                    <CheckCircle size={16} />
