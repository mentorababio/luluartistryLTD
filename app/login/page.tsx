...
export default function LoginPage() {
  ...
  const apiBaseRaw = process.env.NEXT_PUBLIC_API_BASE_URL || 'https://luluartistry-backend.onrender.com/api';
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    if (isLogin) {
      try {
        ...
        // FIX: Look for the token inside data.data.token OR directly inside data.token
        ...
        if (token && typeof token === 'string') {
          ...
          toast.error('Login wrapper mismatch. Checking console...');
          console.log("Backend response format:", data);
        }
      } catch (err) {
    ...
    try {
      ...
      // FIX: Also apply smart token capture on registration if the backend logs them in automatically
      const registerToken = data?.data?.token || data?.token;
      if (registerToken && typeof registerToken === 'string') {
        localStorage.setItem('token', registerToken);
      }

      toast.success('Account created successfully');
      setIsLogin(true); // Switch to login mode smoothly
    } catch (err) {
