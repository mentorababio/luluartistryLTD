...
export default function OrderDetailPage() {
  ...
  const handleDownloadReceipt = () => {
    if (!order) return;
    const orderId = `ORD-${(order._id || order.id || "").slice(-8).toUpperCase()}`;
    const date = new Date(order.createdAt).toLocaleDateString("en-NG", {
      weekday: "long", year: "numeric", month: "long", day: "numeric",
    });
    const items = order.items.map((i) =>
      `  - ${i.name || i.product} x${i.quantity}  ₦${(i.price * i.quantity).toLocaleString()}`
    ).join("\n");
  ...
  if (!order) {
    return (
      <div className="min-h-screen bg-[#fffaf5] flex flex-col items-center justify-center gap-4">
        ...
        <button onClick={() => router.push("/orders")} className="text-[#C9A84C] underline text-sm">Back to Orders</button>
      </div>
    );
  }
      ...
      <div className="max-w-4xl mx-auto px-4 py-8">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
          ...
          <div className="space-y-5">
            ...
            <button onClick={() => router.push("/shop")} className="w-full bg-[#C9A84C] text-white font-semibold py-3 rounded-lg hover:bg-yellow-600 transition-colors flex items-center justify-center gap-2">
              <RotateCcw size={16} />
              Reorder
            </button>
            <button onClick={handleDownloadReceipt} className="w-full border border-[#C9A84C] text-[#C9A84C] font-medium py-3 rounded-lg hover:bg-[#C9A84C] hover:text-white transition-colors flex items-center justify-center gap-2">
              <Download size={16} />
              Download Receipt
            </button>
            <button onClick={() => router.push("/support")} className="w-full border border-gray-200 text-gray-600 font-medium py-3 rounded-lg hover:bg-gray-50 transition-colors flex items-center justify-center gap-2">
                  ...
                  return (
                    <div key={step.key} className="flex gap-4">
                      <div className="flex flex-col items-center">
                        <div className={`w-7 h-7 rounded-full flex items-center justify-center flex-shrink-0 ${isDone ? "bg-green-500" : "bg-gray-100"}`}>
                          {isDone
                            ? <CheckCircle size={18} className="text-white" />
                            : <Circle size={18} className="text-gray-300" />}
                        </div>
                        {!isLast && <div className={`w-0.5 h-8 mt-1 ${isDone && index < currentStep ? "bg-green-400" : "bg-gray-200"}`} />}
                      </div>
                      <div className="pb-6">
                        <p className={`text-sm font-medium ${isDone ? "text-gray-800" : "text-gray-400"}`}>{step.label}</p>
                        {isDone && <p className="text-xs text-gray-400 mt-0.5">{formatDate(order.updatedAt)}</p>}
                        {step.key === "shipped" && order.status === "shipped" && (
                          <span className="inline-flex items-center gap-1 mt-1 text-xs bg-blue-100 text-blue-600 px-2 py-0.5 rounded-full">🚚 In Transit</span>
                        )}
                      </div>
                    </div>
                  );
