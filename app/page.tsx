...
const goToProduct = (productName: string) => {
  ...
  const addToCart = (productId: number) => {
    ...
    if (existingItem) {
      ...
      cartItems.push({
        id: mongoId,          // ← real MongoDB _id, used by checkout as item.product
        name: product.name,
        price: product.price,
        image: product.image,
        quantity: 1,
  ...
  return (
    <div className="bg-[#fffaf5] text-gray-800">
      ...
      <div className="max-w-7xl mx-auto py-16 px-6">
        <div className="mb-8">
          <h1 className="text-3xl md:text-4xl font-bold mb-4">NEW ARRIVAL</h1>
        </div>
    ...
    <div className="p-4">
        ...
        <div className="max-w-7xl mx-auto px-6">
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-4 gap-4">
            {[Instagram1, Instagram2, Instagram3, Instagram4].map((img, i) => (
              <Image key={i} src={img} alt="Instagram Post" width={300} height={450} className="rounded-xl object-cover w-full h-[250px]" />
            ))}
          </div>
        </div>
      </section>
      ...
      <section className="bg-[#fff9ef] py-16">
        <div className="max-w-3xl mx-auto text-center px-6">
          ...
          <p className="text-gray-600 mb-6">Get exclusive beauty tips, product updates, and special offers straight to your inbox.</p>
          <form className="flex flex-col sm:flex-row justify-center gap-4">
            <input type="email" placeholder="Enter your email" className="border border-gray-300 rounded-lg px-4 py-3 w-full sm:w-2/3 focus:ring-2 focus:ring-yellow-500" />
