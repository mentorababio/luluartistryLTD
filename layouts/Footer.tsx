			...
			<footer className='max-w-7xl mx-auto px-6 sm:px-8 py-10 grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-10'>
				...
				<div>
					...
					<div className='flex flex-col gap-2 text-sm'>
						{headerLinks.map((item, i) => {
							if (item.dropdown) {
								// For dropdown items, link to the first dropdown item or shop
								return (
									<Link
										key={i}
										href={item.dropdown[0]?.link || "/shop"}
				...
				<div>
					...
					<div className='flex flex-col gap-2 text-sm'>
						...
						<Link href='/shipping' className='hover:text-primary-gold transition-colors'>
							Shipping Policy
						</Link>
						<Link href='/returns' className='hover:text-primary-gold transition-colors'>
							Return & Refund
						</Link>
